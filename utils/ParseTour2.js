...
for (let i=1;i<=2;i++){
    request(config.url5+i.toString(),function (err, response, body) {
        if (err){
            throw err
        }else {
            $= cheerio.load(body);
...
for (let i=1;i<=2;i++){
    request(config.url6+i.toString(),function (err, response, body) {
        if (err){
            throw err
        }else {
            $= cheerio.load(body);
...
for (let i=1;i<=5;i++){
    request(config.url7+i.toString(),function (err, response, body) {
        if (err){
            throw err
        }else {
            $= cheerio.load(body);
